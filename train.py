...
def main():
    ...
    if utils.is_primary(args):
        ...
        output_dir = utils.get_outdir(args.output if args.output else './output/train', exp_name+args.name)
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        def _forward():
            with amp_autocast():
                ...
                if args.metabalance or args.adatask:
                    loss = loss_fn(output, target,seperate=True)
        ...
        def _backward(_loss):
            ...
            if loss_scaler is not None:
                if args.metabalance:
                            model.metabalance.step(split_loss)
                            #torch.nn.utils.clip_grad_norm_(model.parameters(), 1)
                            #optimizer_ = [optimizer,model.shared_optimizer]
                loss_scaler(
                    _loss,
                    optimizer,
                ...
                if args.metabalance:
                            #pass#
                            #model.shared_optimizer.step()
                            loss_scaler._scaler.step(model.shared_optimizer)
                ...
                if args.adatask:
                    optimizer.backward_and_step(
                        losses=split_loss,
                        shared_parameters=list(model.sharedmodules.parameters()),
                        task_specific_parameters=list(model.taskmodules.parameters()),
                    )
                else:
                    if not args.metabalance:
