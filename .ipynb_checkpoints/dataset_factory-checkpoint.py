...
class SubSampler(Dataset):
    def __init__(self, ds, data_level):
        indices = list(range(len(ds)))
        np.random.seed(42)
        np.random.shuffle(indices)
        split=0
        self.len = int(len(indices) * data_level / 100)
        self.train_idx = indices[split : split + self.len]
        self.ds = ds
...
def create_dataset(
    ...
    if name.startswith('torch/'):
        ...
        # if "aircraft" in name:
        #     if split in _TRAIN_SYNONYM:
        #         name = "hfds/clip-benchmark/wds_fgvc_aircraft"
        #         split = "train"
        #     elif split in _EVAL_SYNONYM:
        #         name = "hfds/clip-benchmark/wds_fgvc_aircraft"
        #         split = "test"
        if "aircraft" in name:
            if split in _TRAIN_SYNONYM:
                name = "hfds/Multimodal-Fatima/FGVC_Aircraft_train"
                split = "train"
            elif split in _EVAL_SYNONYM:
                name = "hfds/Multimodal-Fatima/FGVC_Aircraft_test"
                split = "test"
